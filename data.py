...
'''
% Data: #row = data number, #column = 138 (13+grid_num)，同一时间时间下不同车
%{
0: Dataset Id
1: Vehicle Id
|2 : Frame Id
...
'''
% Tracks: cells: {Dataset_Id * Vehicle_Id}, each cell: #row = 136 (11+grid_num), #column=totalFramNum，固定车辆不同时间的轨迹
%{
|0 : Frame Id
|1 : Local X
|2 : Local Y
...
class highwayTrajDataset(Dataset):
    # t_h过去时间段，t_f未来时间段，10hz，所以过去3s未来5s
    ...
    # getHistory 是方法的名称，它接受一些参数，包括 dsId（数据集ID）、vehId（车辆ID）、
    # refVehId（参考车辆ID）、t（时间）和 wholePeriod（是否使用整个时间段，默认为 False）
    def getHistory(self, dsId, vehId, refVehId, t, wholePeriod=False):
        if vehId == 0:
            ...
            # 如果 vehPos 的长度小于 self.hist_len，则表示历史轨迹长度不足。将 vehPos 与 refPos 的最后 len(vehPos) 个位置相减得到 histPart，
            # 然后使用 np.tile 函数将 histPart 的第一个位置进行复制得到 paddingPart，使得 histPart 的长度与 self.hist_len 相同。
            # 最后，将 paddingPart 和 histPart 沿着轴0进行连接得到完整的历史轨迹 hist。
            # 如果 vehPos 的长度大于等于 self.hist_len，则直接将 vehPos 与 refPos 相减得到 hist。
    ...
    # getPlanFuture 是方法的名称，它接受一些参数，包括 dsId（数据集ID）、planId（计划车辆ID）、refVehId（参考车辆ID）和 t（时间）。
